// src/data/parishes/southern/allSaintsWaatData.js

export const allSaintsWaatData = {
  header: {
    title: "All Saints Waat",
    subtitle: "Patron Saint: All Saints",
    description: "“Go therefore and make disciples of all nations, baptizing them in the name of the Father, and of the Son, and of the Holy Spirit.” Matthew 28:19",
    image: "/images/pageheaders/all-saints-waat-header.png"
  },
  history: {
    priestImage: "/images/parishpriests/fr.bulus.JPG",
    priestName: "Rev. Fr. Bulus Dirpiny",
    description: "All Saints Parish belongs to the Catholic Diocese of Malakal and is located in Waat, Lou Nuer, Jonglei State, South Sudan. It was established through the efforts of local catechists and officially opened by the late Emeritus Bishop Vincent Majwok. The name “All Saints,” proposed by Rev. Fr. Bulus Dirpiny, reflects unity, as Waat means “one,” and is also known in Nuer as Bordiit.The Church in Waat began between 1991 and 1992 as the first Catholic chapel among the Lou Nuer in Waat Payam, Nyirol County. Due to its central location between Nyirol, Uror, and Akobo, it became a pastoral center under Catechist John Mut Lual. However, conflict in Jonglei State disrupted activities as people fled.On 9 August 2019, Bishop Dr. Stephen Nyodho Majwok appointed Rev. Fr. Bulus Dirpiny to resume pastoral ministry in Northern Jonglei. It was officially declared a Parish on 26th, April, 2023, by Bishop Stephen Nyodho Ador."
  },

    otherActivities:[
{
title: "Parish Pastoral Council",
description: "A leadership body that assists the parish priest in planning and coordinating parish activities."
},
{
title: "Youth Ministry",
description: "Formation and fellowship group for youth focusing on faith, responsibility, and leadership in the Church and society. A Catholic youth community that nurtures faith, fellowship, leadership, and service, helping young people grow spiritually and actively participate in the life and mission of the Churc"
},
{
title: "Women Ministry",
description: "A group that brings together women of the parish for prayer, support, and community development activities."
},
{
title: "Pontifical Missionary Childhood",
description: " The Pontifical Missionary Childhood is a Church movement that involves children in missionary work, helping them grow in faith while supporting other children around the world."
},
{
title: "Choir",
description: "A liturgical group that leads the congregation in song during Mass and other celebrations, enriching worship through music while fostering unity, prayer, and active participation in the Church."
},
{
title: "Catechism",
description: "Faith formation classes for children and adults preparing for sacraments such as Baptism, First Communion, and Confirmation."
},
{
title: "Small Christian Communities",
description: "Groups of families who gather regularly for prayer, Bible sharing, and mutual support within the parish."
},
],
  
  parishDetails: {
    location: "Waat, South Sudan",
    deanery: "Southern Deanery",
    patronSaint: "All Saints",
    parishPriest: "Rev. Fr. Bulus Dirpiny",
    otherPriests: "None assigned"
  },
  massSchedules: {
    sundays: ["8:00 AM - English", "10:00 AM - Nuer"],
    weekdays: ["6:00 AM - English", "6:00 PM - English"]
  },
  contact: {
    email: "allsaints.waat@malakaldiocese.org",
    phone: "+211 912 345 702",
    location: "All Saints Parish, Waat, South Sudan",
    officeHours: "Monday - Friday: 8:00 AM - 4:00 PM"
  }
};